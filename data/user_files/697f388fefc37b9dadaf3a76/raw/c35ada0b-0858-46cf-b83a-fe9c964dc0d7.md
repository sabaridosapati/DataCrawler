...
|----------|------|---------|
| Data Loading | `data_loader_node.py` | Extract schema, row count, sample rows |
| Simple Queries | `simple_query_handler_node.py` | Fast SQL for basic aggregations |
| Code Execution | `Python_code_executer_node.py` | Analytics object injected into sandbox |
| Numerical Verification | `numerical_claim_verifier_node.py` | Independent SQL recalculation |
| Data Profiling | `data_profiler_node.py` | Statistical analysis via SQL |
...
|-------------|-------|----------------|
| 0.90-1.00 | `very_high` | Results are highly reliable |
| 0.75-0.89 | `high` | Results are reliable with minor caveats |
| 0.60-0.74 | `moderate` | Results need additional verification |
| 0.40-0.59 | `low` | Results may be inaccurate |
| 0.00-0.39 | `very_low` | Results should not be trusted |
...
| `/api/v1/datasets/upload` | POST | Upload CSV file |
| `/api/v1/analysis/query` | POST | Run analysis query |
| `/api/v1/analysis/query/stream` | POST | SSE streaming |
